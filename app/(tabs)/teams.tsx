...
import { useTeams, useCreateTeamMutation } from "@/lib/queries";
import { uploadToStorage } from "@/lib/db";
...
function CreateTeamModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const submit = async () => {
    ...
    if (imageUri) {
      ...
      try {
        const url = await uploadToStorage(imageUri, `teams/${Date.now()}.jpg`, "image/jpeg");
    ...
    createTeam(
      { name: name.trim(), sport, season: season.trim() || undefined, logoUrl: imageUrl },
      {
        onSuccess: () => {
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item: team }) => (
            ...
            >
              {team.logoUrl ? (
                <Image source={{ uri: team.logoUrl }} style={styles.avatarImg} />
              ) : (
                <View style={[styles.avatar, { backgroundColor: team.primaryColor ?? colors.primary }]}>
