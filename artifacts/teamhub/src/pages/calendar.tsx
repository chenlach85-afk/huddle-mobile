...
const TYPE_CONFIG: Record<string, { color: string; label: string }> = {
  ...
  other: { color: "rgba(255,255,255,0.4)", label: "Other" },
};
    ...
    <div className="p-5 md:p-8 max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="font-display text-4xl text-white tracking-wide">CALENDAR</h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className="px-3 py-1.5 rounded-xl text-xs font-bold tracking-wider transition-all"
            style={{
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-end mb-1">
                      <span
                        ...
                        style={{
                          ...
                          fontFamily: isTodayDay ? "var(--app-font-display)" : "inherit",
                    ...
                    <div className="space-y-0.5">
                      ...
                      {dayEvents.length > 3 && (
                        <div className="text-[10px] text-white/30 font-bold px-1.5">
          ...
          {selectedDay ? (
            <div className="rounded-2xl border border-white/6 overflow-hidden"
              ...
              <div className="px-4 py-3 border-b border-white/6 flex items-center justify-between">
                <div>
                  <p className="section-label">{format(selectedDay, "EEE").toUpperCase()}</p>
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs font-bold uppercase tracking-wider"
                          style={{ color: TYPE_CONFIG[ev.type]?.color ?? ev.teamColor }}>
                          {TYPE_CONFIG[ev.type]?.label ?? ev.type}
                        </span>
                      </div>
                      <p className="text-sm font-semibold text-white truncate">{ev.title}</p>
                      <p className="text-xs text-white/40 mt-0.5">{format(parseISO(ev.startsAt), "h:mm a")}</p>
                    ...
                    .map(ev => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: ev.teamColor }}>
                            {ev.teamName}
                          </span>
                        </div>
                        <p className="text-sm font-semibold text-white truncate">{ev.title}</p>
                        <p className="text-xs text-white/35">
                          {format(parseISO(ev.startsAt), "EEE, MMM d · h:mm a")}
                        </p>
                      </button>
                    ))}
          ...
          {selectedEvent && (
            <div className="rounded-2xl border overflow-hidden"
              ...
              <div className="px-4 py-3 border-b flex items-center justify-between"
                ...
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-widest"
                    style={{ color: TYPE_CONFIG[selectedEvent.type]?.color ?? selectedEvent.teamColor }}>
              ...
              <div className="px-4 py-3 space-y-2">
                <p className="font-display text-xl text-white tracking-wide leading-tight">
                ...
                <div className="space-y-1.5 pt-1">
                  <div className="flex items-center gap-2 text-xs text-white/60">
                    ...
                    <span>{format(parseISO(selectedEvent.startsAt), "EEE, MMM d · h:mm a")}</span>
                  </div>
                  {selectedEvent.endsAt && (
                    <div className="flex items-center gap-2 text-xs text-white/40">
                      <div className="w-3.5 shrink-0" />
                      <span>Until {format(parseISO(selectedEvent.endsAt), "h:mm a")}</span>
                    </div>
                  )}
